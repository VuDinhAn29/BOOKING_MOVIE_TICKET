...
function Checkout(props) {
  ...
  const { chiTietPhongVe, danhSachGheDangDat, danhSachGheKhachDat } = useSelector(state => state.QuanLyDatVeReducers);
  ...
  const renderSeats = () => {
    return danhSachGhe.map((ghe, index) => {
      ...
      let classGheDangDat = '';
      //Kiểm tra từng ghế render xem có trong mảng ghế đang đặt hay kh
      let indexGheDD = danhSachGheDangDat.findIndex(gheDD => gheDD.maGhe === ghe.maGhe);
      if (indexGheDD != -1) {
        classGheDat = 'gheDangDat';
      }
      ...
      return <Fragment key={index}>
        <button onClick={() => {
          ...
          dispatch(action);
        }} disabled={ghe.daDat || classGheKhachDat != ''} className={`ghe ${classGheVip} ${classGheKhachDat} ${classGheDat} ${classGheDangDat} ${classGheDaDuocDat} text-center`} key={index}>
          {ghe.daDat ? classGheDaDuocDat != '' ? <UserOutlined style={{ marginBottom: 7.5, fontWeight: 'bold' }} /> : <CloseOutlined style={{ marginBottom: 7.5, fontWeight: 'bold' }} /> : classGheKhachDat !== '' ? <  SmileOutlined /> : ghe.stt}
        </button>
        {(index + 1) % 16 === 0 ? <br /> : ''}
      </Fragment>
    })
      ...
      <div className='grid grid-cols-12'>
        ...
        <div className='col-span-3 '>
          ...
          <div className='flex flex-row my-5'>
            <div className='w-4/5'><span className='text-red-400 mr-2'>Ghế</span>

              {_.sortBy(danhSachGheDangDat, ['stt']).map((gheDD, index) => {
                return <span key={index} className='text-green-500 text-xl mr-3'>{gheDD.stt}</span>
              })}

            </div>
            <div className='text-right col-span-1'><span className='text-red-800 text-lg'>
              {danhSachGheDangDat.reduce((tongTien, ghe, index) => {
                return tongTien += ghe.giaVe;
              }, 0).toLocaleString()}
            </span>
...
export default function CheckoutTab(props) {
  ...
  return <Tabs className='p-5' tabBarExtraContent={operations} activeKey={tabActive} onChange={(key) => {
    dispatch({
      type: CHANGE_TAB_ACTIVE,
      number: key.toString(),
    })
    ...
    <Tabs.TabPane tab={<div className="text-center" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}><NavLink to="/"><HomeOutlined style={{ marginLeft: 10, fontSize: 25 }} /></NavLink></div>} key="3"></Tabs.TabPane>
